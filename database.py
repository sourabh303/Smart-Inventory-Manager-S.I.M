...
def get_stats():
    # ⚡ Bolt: Replaced 7 separate queries with 2 using conditional aggregation
    with get_db() as conn:
        ...
        return {
            "total_items":     inv_row["total"] if inv_row else 0,
            "available":       inv_row["available"] if inv_row else 0,
            "low_stock":       inv_row["low"] if inv_row else 0,
            "out_of_stock":    inv_row["out"] if inv_row else 0,
            "ordered":         inv_row["ordered"] if inv_row else 0,
            "pending_requests": req_row["pending_req"] if req_row else 0,
            "ordered_requests": req_row["ordered_req"] if req_row else 0,
        }
